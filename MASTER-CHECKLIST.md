...
1. [Next Steps Plan](planning/next-steps-plan.md) — historical scope and full dependency plan.
2. [Epoch 1 Foundation Decisions](decisions/2026-07-10-epoch-1-foundation.md) — foundation decisions; linked decision records win on conflicts.
3. [Current State](status/CURRENT-STATE.md) and dated observation records — current evidence.
4. [Context Map](CONTEXT-MAP.md) — re-entry dependency maps and reference
   index; explanatory only, never an execution authority.
5. [Charter](CHARTER.md) — canonical Epoch 1 authority and amendments.
6. [Decision Register](decisions/DECISION-REGISTER.md) — the
...
- **Corpus reaches [[runs/run-011/RUN-011-Comedy-Concept|RUN-011]]:** Runs 005–009 retain the care-delivery account,
  adjacency, influence, narrative/pain, and candidate-DSP evidence; [[runs/run-010/RUN-010-Data-Lineage|RUN-010]]
  indexes the corpus and [[runs/run-011/RUN-011-Comedy-Concept|RUN-011]] qualifies one internal founder-theme route.
  None establishes demand or authorizes outreach or publishing. [Current state](status/CURRENT-STATE.md)
- **Care-delivery language stays bounded:** [[runs/run-008/RUN-008-Narrative-Pain-Map|RUN-008]] recurrence is source-profile
  coding, not audience prevalence or demand evidence, and [[runs/run-009/RUN-009-DSP-Qualification|RUN-009]]'s candidate
...
- **Lineage is now checkable:** [[runs/run-010/RUN-010-Data-Lineage|RUN-010]] maps retained entities, IDs, artifacts,
  and dependencies through [[runs/run-009/RUN-009-DSP-Qualification|RUN-009]] without adding strategic recommendations.
  [Lineage map](EPOCH-1-HIERARCHY-DATA-LINEAGE.md)
- **First social route is complete:** Mark selected the cross-cutting one-lab
  lane and founder-theme route; [[runs/run-011/RUN-011-Comedy-Concept|RUN-011]] advanced `SOC-008` only to a separately
...
- [x] `A1` **$0 external-model cap and $0 sandbox cap** — [[decisions/2026-07-10-epoch-1-foundation|DR-8]]. Do not raise or
  work around either cap without a new dated Mark decision. *2026-07-11 note
  ([[decisions/2026-07-11-run-001-approvals|DR-16]]): Mark's personal manual testing is controlled by the existing
...
- [x] `B3` **D4 documentation:** root `AGENTS.md` names the live dashboard
  ...
  MindfulText core is now `qwen3:8b` ([[decisions/2026-07-12-mark-directions|DR-19]]).
- [x] `B4` **Log observability:** dashboard restart at `2026-07-10T14:38:25.508Z`
  enables ISO-8601 log prefixes (commit `c7f0b32`).
- [x] `B5` **Canonical charter and dated decisions:** charter v1.1 and [[decisions/2026-07-10-epoch-1-foundation|DR-1]]…[[decisions/2026-07-10-epoch-1-foundation|DR-12]]
...
- [x] `B6c` Unknown/unverified provider surfaces are external; named provider,
  per-run cap, and Mark approval are required (charter §14.3; [[decisions/2026-07-10-epoch-1-foundation|DR-8]]).
...
- [x] `B6m` Path-scoring rubric v0.1 is drafted at
  [Growth Scorecard](growth/SCORECARD.md) (charter
  §14.13); Mark approval remains a Run 001 gate.
- [x] `B6n` Pi remains optional-later, not the Epoch 1 control plane (charter §14.14;
  [[decisions/2026-07-10-epoch-1-foundation|DR-7]]).
- [x] `B6o` Targets are re-dated: Aug 14 packet / Sep 4 signal (charter §3; [[decisions/2026-07-10-epoch-1-foundation|DR-6]]).
...
- [x] `C1b` **Mark chose the final non-iCloud lab location:**
  `~/AI-Studio/Projects/mindfultext-epoch-1/` ([[decisions/2026-07-10-epoch-1-foundation|DR-9]]), confirmed 2026-07-10.
...
- [x] `C1c` **Mark only — choose a separate local-only store** outside any tracked
  ...
  non-cloud-synced, and no fallback if unavailable ([[decisions/2026-07-12-mark-directions|DR-20]]).
- [x] `C1d` `datasets/` is ignored; no raw evidence is present in this repo.
- [x] `C1e` §6.3 new-repo waiver and non-iCloud rationale are recorded in [[decisions/2026-07-10-epoch-1-foundation|DR-9]];
...
- [x] `C2a` **Mark only — classify or strike every candidate in
  [Growth Hypotheses](growth/HYPOTHESES.md):** validation notes,
  pilot feedback (including third-party consent), ICP one-liner, and product
  positioning. *Approved by Mark 2026-07-11 — all four retained ([[decisions/2026-07-11-run-001-approvals|DR-13]]).*
- [x] `C2b` **Mark only — complete rights/consent, data class, permitted use, and
  ...
  [[decisions/2026-07-11-run-001-approvals|DR-13]]) — spot-check each actual asset at import, esp. asset 2's third-party
  content.*
- [x] `C2c` **Mark only — add only cleared summaries/provenance to Approved
  ...
  Mark 2026-07-11 ([[decisions/2026-07-11-run-001-approvals|DR-13]]); populates when assets are physically imported.*
- [x] `C2d` **Mark only — resolve the Run 001 collision:** set its input set to
  ...
  set classed `external-okay` ([[decisions/2026-07-11-run-001-approvals|DR-13]]).*
...
- [x] `C3b` **Mark only — review/amend rubric v0.1** in
  [Growth Scorecard](growth/SCORECARD.md), including
  ...
  unamended ([[decisions/2026-07-11-run-001-approvals|DR-14]]).*
- [x] `C3c` **Mark only — review/amend Team Config v0.1:** Claude-only external
  ...
  [[decisions/2026-07-12-mark-directions|DR-17]]/[[decisions/2026-07-12-prototype-run-first|DR-22]] manual prototype policy; Telegram stays out of the workflow.*
- [x] `C3d` **Mark only — complete and approve Run 001 card:** set `data_class`,
  ...
  YYYY-MM-DD`. *Approved by Mark 2026-07-11 ([[decisions/2026-07-11-run-001-approvals|DR-16]]); clarified by [[decisions/2026-07-12-mark-directions|DR-17]] on
  2026-07-12.*
...
- [x] `C4c` **Mark only — direct the D2 response:** Mark authorized and Codex
  deployed the core swap from `gpt-oss:20b` to `qwen3:8b` ([[decisions/2026-07-12-mark-directions|DR-19]]; dashboard
  commit `7b1cec0`). Live health confirms `core_degraded: false`; [[decisions/2026-07-12-prototype-run-first|DR-22]] makes
...
- [ ] `C4d` **Deferred background telemetry:** when convenient, record a fresh
  ...
  It does not gate Run 001 or Run 002 ([[decisions/2026-07-12-prototype-run-first|DR-22]]).
...
- **Check run cost and latency:** The Run 001 trace records bounded completion but no reliable harness duration/usage figure. Record that gap; do not add a second project budget system. [Run 001 card](runs/run-001/RUN-001-Growth-Path-Ranking.md)

#### D5 — Approve Run 002

- **Choose the research scope:** Mark approved Run 002 for `SEG-001 — Care-delivery organizations`, using public account intelligence and local research evaluation rather than a pure model comparison. [Run 002 card](runs/run-002/RUN-002-Segment-Intelligence.md), [DR-29](decisions/2026-07-13-segment-linked-runs.md)
...
- **Execute and trace safely:** Produce cited public segment/account/buyer/language/category research and a reference/hash-only trace. An eviction or `503` makes only the local-work evaluation inconclusive; public research may continue. [Run 002 card](runs/run-002/RUN-002-Segment-Intelligence.md)
...
- [x] `D1` **Run 001 — hypothesis exploration ([[decisions/2026-07-12-direct-growth-hypotheses|DR-24]]/[[decisions/2026-07-12-research-controls-and-profile-watch|DR-25]]/[[decisions/2026-07-12-tam-first-market-sizing|DR-27]]/[[decisions/2026-07-12-icp-registry|DR-28]]):** a Mark-prompted agent starts from
  ...
  `agent-hypothesis`; stop at five cards or four hours. *Completed 2026-07-12 by Codex for [[growth/HYPOTHESES|GH-002]]: five paths, ranking, gaps, public-profile watch, and a cited U.S. TAM note are linked from the hypothesis; no outbound activity occurred.*
- [x] `D2` Run 001 records the charter version and commit in its card at execution. *Recorded 2026-07-12 in `runs/run-001/run-card.md`: charter v1.9; baseline commit `e42a302`; Codex named by Mark for the [[growth/HYPOTHESES|GH-002]] invocation.*
- [x] `D3` **Mark — review Run 001 evidence quality and decision usefulness.** The MHA Dutchess context makes the organization-sponsored direction useful enough to prioritize, while transferability remains unproven ([[decisions/2026-07-13-segment-linked-runs|DR-29]]).
- [x] `D4` **Mark — review Run 001 usage record.** Five paths were completed within the card's bounded session; the trace does not supply a reliable duration/usage figure. That gap is recorded and does not create another project budget system ([[decisions/2026-07-13-segment-linked-runs|DR-29]]).
- [x] `D5` **Mark — approve Run 002:** segment intelligence and local research evaluation for `SEG-001`, not a pure model benchmark ([[decisions/2026-07-13-segment-linked-runs|DR-29]]).
...
- [x] `D7` **Run 002 — execute public segment intelligence:** cited account, buyer-group, market-language, category, contradiction, and `GH-004` outputs are present. The local `task:classify` check returned no usable response, so local-work evaluation is **inconclusive**; public research remains usable. [Run 002 blackboard](runs/run-002/RUN-002-Segment-Research-Notes.md)
- [x] `D8` **Mark — review Run 002 research and local-work value.** Mark advanced the focused community-behavioral-health branch; the local check was inconclusive and does not determine the market result ([[decisions/2026-07-13-run-003-paired-model-comparison|DR-30]]).
- [x] `D9` **Select one Run 003 test:** `GH-004` / `SEG-002` is selected for a paired local/frontier validation packet. The frontier provider/model must be named before that arm runs; both arms use the same frozen input and no outreach is authorized ([[decisions/2026-07-13-run-003-paired-model-comparison|DR-30]]).

## Phase 3 — Run 003 and decision fork (by 2026-08-14)

- [x] `E1` **Run 003 — paired validation packet:** `CMP-001-F` and `CMP-001-L` both completed with the frozen input; the scorecard records the captured local result and the safer frontier packet. Mark review remains required and neither output authorizes outreach ([[decisions/2026-07-13-run-003-paired-model-comparison|DR-30]]).
- [x] `E2` **Define the selected test:** the frontier packet defines a level-2 buyer-intent signal, pass/fail/stop conditions, and the prerequisite public account qualification. Mark still decides whether any manual conversation is warranted ([[decisions/2026-07-13-run-003-paired-model-comparison|DR-30]]).
- [x] `E2a` **Create segment-linked customer-development plan:** `CDP-001` links parent `SEG-001`, focused branch `SEG-002`, `GH-004`, and `GTM-001`. It maps bounded target intelligence and marketing opportunities but authorizes no research run, outreach, publication, monitoring, or manual conversation ([[decisions/2026-07-14-customer-development-plan|DR-31]]).
- [x] `E2b` **Complete — [[runs/run-004/RUN-004-Local-Research-Sprints|RUN-004]] [[growth/CDP-001|CDP-001]] local-research sprint program:** 27
  ...
  outreach, publishing, provider fallback, or a residency change ([[decisions/2026-07-14-run-004-cdp-local-research|DR-32]]).
- [x] `E2c` **Complete — [[runs/run-005/RUN-005-Organization-Registry|RUN-005]] care-delivery organization registry:** 56
  ...
  any organization ([[decisions/2026-07-15-run-005-care-delivery-registry|DR-33]]).
- [x] `E2d` **Complete — [[runs/run-006/RUN-006-One-Degree-Market-Map|RUN-006]] care-delivery one-degree map:** 50 source-backed
  ...
  code fork, code merge, CRM write, or external action occurred ([[decisions/2026-07-15-run-006-one-degree-map|DR-34]]).
- [x] `E2e` **Complete — [[runs/run-007/RUN-007-Influence-Map|RUN-007]] care-delivery influence map:** 50 public
...
- [x] `E2f` **Complete — [[runs/run-008/RUN-008-Narrative-Pain-Map|RUN-008]] narrative and pain map:** Approved fork of
  [[runs/run-007/RUN-007-Influence-Map|RUN-007]] attempted all 50 public sources and retained a narrative registry,
...
- [x] `E2g` **Complete — [[runs/run-009/RUN-009-DSP-Qualification|RUN-009]] Direct Support Professional ICP qualification:**
...
- [x] `E2h` **Record the gap-analysis roadmap and feature backlog:** the
  ...
  ([[decisions/2026-07-15-gap-analysis-roadmap-and-feature-backlog|DR-35]]).
- [x] `E2i` **Complete — [[runs/run-010/RUN-010-Data-Lineage|RUN-010]] Epoch 1 hierarchy and data lineage:** the
  source-linked map inventories retained entities, identifiers, artifacts, and
  dependencies through [[runs/run-009/RUN-009-DSP-Qualification|RUN-009]] without synthesizing findings or making
...
- [x] `E2j` **Record social-intelligence and founder-content direction:**
  ...
  [Lane outline](checklists/SOCIAL-LEARNING-LANE.md) ([[decisions/DECISION-REGISTER|DR-36]]).
- [x] `E2k` **Complete — Mark chose the social placement and first decision:**
  ...
  execute the `SOC-008` internal positioning pass by X3. [[runs/run-011/RUN-011-Comedy-Concept|RUN-011]] advances a
  ...
  [Concept packet](runs/run-011/outputs/SOC-008-CONCEPT-PACKET.md) ([[decisions/DECISION-REGISTER|DR-36]]).
...
4. `G4` **Mark — review Run 002 (D8):** assess `SEG-002` research, the candidate-account universe, `GH-004`, and whether local work added enough value to retry. [Run 002 blackboard](runs/run-002/RUN-002-Segment-Research-Notes.md), [accounts](growth/ACCOUNT-INTELLIGENCE.md).
...
6. `G6` **Mark — review [[runs/run-006/RUN-006-One-Degree-Market-Map|RUN-006]]:** decide whether the one-degree map warrants an atomic `SEG-003` comparison hypothesis, a focused channel/association study, or a pause. [RUN-006 report](runs/run-006/outputs/RUN-006-REPORT.md), [map](runs/run-006/outputs/one-degree-map.csv)
