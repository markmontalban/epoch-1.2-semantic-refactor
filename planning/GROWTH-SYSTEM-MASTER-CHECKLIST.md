...
Last updated: 2026-08-17 22:25:00 PDT — edited by: Cursor

Current-state reconciliation: 2026-08-17 22:25:00 PDT — edited by: Cursor
...
| W3 Growth CRM/API | in progress | W3b disposable acceptance stands; `W3d-1`–`W3d-8` remain laptop-complete. Mark confirmed Growth Ops Preview already uses `W3b-testing`. A Preview-only public-site credential labeled `mindfultext-web-preview-w4b` was hashed into that disposable branch and `GROWTH_OPS_BASE_URL` / `GROWTH_OPS_SITE_CREDENTIAL` were added to `mindfultext-web` Preview (`w1-clean-shell`). Authenticated Preview acceptance is still open: Growth Ops Preview returns Vercel SSO `302`, and no owner-issued test word-only link has been walked. No Production credential or collection. |
| W4 Public beta site | in progress | `W4a-L1a`–`W4a-L2b` direction approved; `W4b-L1a`/`L1b` and `W4b-L2a`/`L2b`/`L2c` laptop-only done per [DR-56](../decisions/2026-08-17-w4b-l1-build-confirmation-and-l2-continue.md); `W4b-L1c` and `W4b-L2d` private-test authorized by [DR-57](../decisions/2026-08-17-w4b-l1c-l2d-private-test.md) and not yet accepted; L3/L4 remain deferred |
...
| W4b-L1c | [ ] Pass L1 word-handle, lifecycle, forwarding, privacy, mobile, accessibility, logging, outage, and authenticated Preview acceptance.                                                                                                  | Agent | Authorized by [DR-57](../decisions/2026-08-17-w4b-l1c-l2d-private-test.md); not yet accepted. Preview-only site credential issued into `W3b-testing` (label `mindfultext-web-preview-w4b`); public-site Preview env on git branch `w1-clean-shell` holds `GROWTH_OPS_BASE_URL` and `GROWTH_OPS_SITE_CREDENTIAL`. Growth Ops Preview `/api/readiness` still returns Vercel SSO `302`, so the public site cannot call it until Protection Bypass for Automation is enabled. Uncommitted `mindfultext-web` helper can send `x-vercel-protection-bypass` once that secret exists. No Production or public collection. |
...
[DR-57](../decisions/2026-08-17-w4b-l1c-l2d-private-test.md) authorizes
W4b-L1c/W4b-L2d private-test checks. Laptop polish for those gates (cookie
notice beside the DR-55 purpose notice, visitor-safe forwarding copy, outage
copy, larger tap targets, focus/retry) is in `mindfultext-web` as of
2026-08-17 20:50 PDT; 43 public-site tests and typecheck passed. Authenticated
Preview evidence is not yet recorded. 2026-08-17 22:25 PDT: Preview-only site
credential and `w1-clean-shell` env vars are in place; Vercel SSO on Growth Ops
Preview still blocks `W4b-L1c` / `W4b-L2d` server calls.
